...
| Multi-journey support | ✅ Real: a data grid (`JourneysListPage`) lists every journey, each with its own full document under its own `localStorage` key; create/edit/delete all work; navigation persists across refresh via a hash-based URL (`lib/route.ts`); search/sort/pagination via `useListControls` | No folders or tags |
...
4. [x] **Search/filter/sort/pagination on the Journeys and Events grids.** Fixed: `lib/listControls.ts` (case-insensitive multi-field search, stable sort, page clamping) is a plain, unit-tested module shared by both grids via a thin `useListControls` hook and `components/shared/GridControls.tsx` (search box, sortable `<th>`, pagination footer) — the same three pieces back both pages rather than each grid rolling its own. Search matches name/label + description; column headers cycle asc → desc → unsorted; page size is 10. 17 new tests in `listControls.test.ts`.
